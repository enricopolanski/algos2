...
describe('onDisconnect', () => {
  ...
  it('should have one connection after setting up a connection', () => {
    expect(chatModule.setupConnection(Object.create(null))).toBe('user-1');
    expect(chatModule.connections.length).toBe(1);
  });
  ...
  const broadCastSpy = jest.spyOn(chatModule, 'broadcast');

  it("the broadcast hasn't been called yet", () => {
